...
export const addRouteLayer = async (map, startTravel, endTravel) => {
  try {
    const response = await fetch('/assets/route_palestine.geojson'); // Ensure the path is correct
  ...
  for (let i = 0; i < routeData.features.length; i++) {
    ...
    if (feature.geometry.type === 'LineString') {
      for (let j = 0; j < feature.geometry.coordinates.length - 1; j++) {
        ...
        const distanceStartStart = areCoordsClose(segmentStart, startCoord);
        const distanceEndStart = areCoordsClose(segmentEnd, startCoord);
        if (distanceStartStart && distanceEndStart) {
          filteredRoute.push(feature);
          break;
        } else {
          const distanceStartEnd = areCoordsClose(segmentStart, endCoord);
          const distanceEndEnd = areCoordsClose(segmentEnd, endCoord);
 ...
 */
export const areCoordsClose = (coord1, coord2, threshold = 4) => {
